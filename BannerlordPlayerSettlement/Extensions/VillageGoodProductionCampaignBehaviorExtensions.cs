...
namespace BannerlordPlayerSettlement.Extensions
{
    public static class VillageGoodProductionCampaignBehaviorExtensions
    {
        ...
        {
            Settlement settlement = SettlementHelper.FindNearestSettlementToSettlement(village.Settlement, TaleWorlds.CampaignSystem.Party.MobileParty.NavigationType.Default, (Settlement x) =>
            {
                if (!x.IsTown)
                {
                    return false;
            ...
            if (settlement != null && Campaign.Current.Models.MapDistanceModel.GetDistance(settlement, village.Settlement, false, false, TaleWorlds.CampaignSystem.Party.MobileParty.NavigationType.Default) < 150f)
            {
                village.SetTradeBound(settlement);
                return;
            }
            Settlement settlement1 = SettlementHelper.FindNearestSettlementToSettlement(village.Settlement, TaleWorlds.CampaignSystem.Party.MobileParty.NavigationType.Default, (Settlement x) =>
            {
                if (!x.IsTown || x.Town.MapFaction == village.Settlement.MapFaction || x.Town.MapFaction.IsAtWarWith(village.Settlement.MapFaction))
                {
                    return false;
                }
                return Campaign.Current.Models.MapDistanceModel.GetDistance(x, village.Settlement, false, false, TaleWorlds.CampaignSystem.Party.MobileParty.NavigationType.Default) <= 150f;
            });
            if (settlement1 != null && Campaign.Current.Models.MapDistanceModel.GetDistance(settlement1, village.Settlement, false, false, TaleWorlds.CampaignSystem.Party.MobileParty.NavigationType.Default) < 150f)
            {
                village.SetTradeBound(settlement);
                return;
            }
